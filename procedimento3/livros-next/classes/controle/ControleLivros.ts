...
export default class ControleLivros {
  ...
  public incluir(livro:Livro):void {
    console.log('dentro de ControleLivros',livro,livros)
    let novoCod = livros[livros.length - 1].codigo + 1;
    livro.codigo=novoCod
    livros.push(livro as any);
  }
 

  public excluir(cod: number): void {
     
    console.log('excluir livro controle entrada cod==>',cod,livros)
    let index = livros.findIndex((item) => item.codigo === cod);
    if (index >= 0) {
